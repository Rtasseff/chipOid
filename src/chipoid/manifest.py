...
def load_manifest(path: Path) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(
            f"manifest {path} missing required column(s): {sorted(missing)}. "
            f"Found: {list(df.columns)}"
        )

    # Strip whitespace from string cells in the required columns.
    df["image_id"] = df["image_id"].astype(str).str.strip()
    df["source"] = df["source"].astype(str).str.strip()

    if df["image_id"].duplicated().any():
        dupes = df.loc[df["image_id"].duplicated(), "image_id"].tolist()
        raise ValueError(f"manifest has duplicate image_id values: {dupes}")
